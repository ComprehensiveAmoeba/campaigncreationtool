...
def validate_data(data):
    ...
    for i, row in data.iterrows():
        asin = str(row['ASIN'])
        if not (asin.startswith(('B0', 'b0')) and len(asin) == 10):
...
def generate_campaign_file(data, cross_negation, all_quartile_unmanaged):
    output_rows = []
    for _, row in data.iterrows():
        ...
        if all_quartile_unmanaged or (pd.notna(row['Unmanaged by Quartile?']) and row['Unmanaged by Quartile?'].strip().upper() == "YES"):
            campaign_name = "UN_" + campaign_name
...
st.markdown("""**Instructions:** Please fill in the template available [here](https://docs.google.com/spreadsheets/d/1EfaegRfxOOclnPptF2u5xnLvylG634mQKqwqXQDMfcg/edit?gid=0#gid=0) and download a CSV copy.""")
...
if uploaded_file is not None:
    ...
    if errors:
        ...
        all_quartile_unmanaged = st.checkbox("All Quartile unmanaged")
        if st.button("Generate Campaign File"):
            output_df = generate_campaign_file(data, cross_negation == "Yes", all_quartile_unmanaged)
            towrite = BytesIO()
